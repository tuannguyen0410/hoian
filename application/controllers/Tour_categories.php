...
class Tour_categories extends CI_Controller {
	...
	public function __construct(){
		parent::__construct();
		$this->load->model('tour_categories_model','model');
	...
	public function admincp() {
		...
		$data = array(
			'result' => $this->model->getListContent($this->category,array('parent_id' => 0, 'type' => 'tour'), 'category_id asc, order_by asc', null, null, "id, name, name_en, status,images, order_by, category_id"),
			'category' => $this->model->getListContent($this->category, array('parent_id' => 0, 'type' => 'tour'), 'order_by asc', null, null, "name, id")
		);
		$this->template->write_view('content','BACKEND/tour_categories/content', $data);
	...
	public function addContent() {
		...
		$this->template->write_view('content','BACKEND/tour_categories/edit',$data);
		$this->template->render();
	}
	...
	public function saveContent() {
		if($_POST) {
			...
			if($this->model->saveContent($this->category, $data)){
				...
				echo PATH_URL.'acp/tour/categories/edit/'.$id;
			}
		}
	}
	public function delete() {
		if(isset($_POST['orderButton'])) {
			...
			header('location:'.PATH_URL.'acp/tour/categories');
			exit();
		}
	...
	public function searchContent($category) {
		$data = array(
            'category' => $this->model->getListContent($this->category, array('parent_id' => 0, 'type' => 'tour'), "order_by asc", null,null, "name, id"),
			'result' => $this->model->getListContent($this->category, "FIND_IN_SET($category,category_id)>0 AND type = 'tour'", "order_by asc", null, null, "images,name, id, status, order_by, category_id, links")
        );
		$this->template->write_view('content','BACKEND/tour_categories/content', $data);
		$this->template->render();
	}
	...
	public function checkLinks()
    {
		...
		if($id == '') {
			$order_by = (int) $this->db->get_where($this->category,array('type' => 'tour'))->num_rows() + 1;
		} else {
