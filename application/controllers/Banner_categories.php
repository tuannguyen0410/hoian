...
class Banner_categories extends CI_Controller {
	private $user = "user";
	private $category = 'category';
	private $setting = "setting";
	public function __construct(){
		parent::__construct();
		$this->load->model('banner_categories_model','model');
	...
	public function admincp() {
		...
		$data = array(
			'result' => $this->model->getListContent($this->category,array('parent_id' => 0, 'type' => 'banner'), 'category_id asc, order_by asc', null, null, "id, name, name_en, status,images, order_by, category_id"),
			'category' => $this->model->getListContent($this->category, array('parent_id' => 0, 'type' => 'menu'), 'order_by asc', null, null, "name, id")
		);
		$this->template->write_view('content','BACKEND/banner_categories/content', $data);
	...
	public function addContent() {
		...
		$this->template->write_view('content','BACKEND/banner_categories/edit',$data);
		$this->template->render();
	}
	...
	public function saveContent() {
		if($_POST) {
			...
			if($_FILES['images']['name'] != '') {
				if($id != '') {
					$img = $this->model->getContent($this->category, array('id' => $id), 'images');
			...
			if($this->model->saveContent($this->category, $data)){
				...
				echo PATH_URL.'acp/banner/categories/edit/'.$id;
			}
		}
	}
	public function delete() {
		if(isset($_POST['orderButton'])) {
			...
			header('location:'.PATH_URL.'acp/banner/categories');
			exit();
		}
	...
	public function searchContent($category) {
		$data = array(
            'category' => $this->model->getListContent($this->category, array('parent_id' => 0, 'type' => 'menu'), "order_by asc", null,null, "name, id"),
			'result' => $this->model->getListContent($this->category, "FIND_IN_SET($category,category_id)>0 AND type = 'banner'", "order_by asc", null, null, "images,name, id, status, order_by, category_id, links")
        );
		$this->template->write_view('content','BACKEND/banner_categories/content', $data);
		$this->template->render();
	}
	...
	public function checkLinks()
    {
		...
		if($id == '') {
			$order_by = (int) $this->db->get_where($this->category,array('type' => 'banner'))->num_rows() + 1;
		} else {
