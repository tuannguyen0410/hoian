...
class Network extends CI_Controller {
	...
	public function admincp() {
		...
		$data = array(
			...
			'result' => $this->model->getListContent($this->article,array('type' => 'network'), 'category_id asc, order_by asc, updated desc', null, null, "images,name,name_en,id,status,order_by,category_id,links,homepage,featured")
		);
		$this->template->write_view('content','BACKEND/network/content', $data);
	...
	public function addContent() {
		...
		$this->template->write_view('content','BACKEND/network/edit',$data);
		$this->template->render();
	}
	...
	public function saveContent() {
		if($_POST) {
			...
			$data = array(
				...
				'shortcontent' => addslashes($this->input->post('txtAddress')),
				'content' => addslashes($this->input->post('content')),
				'title' => trim(addslashes($this->input->post('txtCoordinates'))),
				'description' => trim(addslashes($this->input->post('txtPhone'))),
				'order_by' => $order_by,
			...
			if($this->model->saveContent($this->article,$data)){
				...
				echo PATH_URL.'acp/network/edit/'.$id;
			}
		}
	}
	public function delete() {
		if(isset($_POST['orderButton'])) {
			...
			header('location:'.PATH_URL.'acp/network');
			exit();
		}
	...
	public function searchContent($category) {
		$data = array(
            'category' => $this->model->getListContent($this->category, array('parent_id' => 0, 'type' => 'page'), "order_by asc", null,null, "name, id"),
			'result' => $this->model->getListContent($this->article, "FIND_IN_SET($category,category_id)>0 AND type = 'network'", "order_by asc", null, null, "images,name, id, status, order_by, category_id, links, homepage, featured")
        );
		$this->template->write_view('content','BACKEND/network/content', $data);
		$this->template->render();
	}
	...
	public function checkLinks()
    {
		...
		if($id == '') {
			$order_by = (int) $this->db->get_where($this->article, array('type' => 'network'))->num_rows() + 1;
		} else {
