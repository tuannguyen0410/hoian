...
class Testimonial_categories extends CI_Controller {
	private $user = "user";
	private $category = 'category';
	private $setting = "setting";
	public function __construct(){
		parent::__construct();
		$this->load->model('testimonial_categories_model','model');
		...
		if(strtolower($this->uri->segment(1)) == 'acp') {
			...
			$left = array(
				'active' => 'testimonial',
				'category' => 'testimonial_categories'
			);
	...
	public function admincp() {
		...
		$data = array(
			'result' => $this->model->getListContent($this->category,array('parent_id' => 0, 'type' => 'testimonial'), 'category_id asc, order_by asc', null, null, "id, name, name_en, status,images, order_by"),
			'category' => $this->model->getListContent($this->category, array('parent_id' => 0, 'type' => 'page'), 'order_by asc', null, null, "name, id")
		);
		$this->template->write_view('content','BACKEND/testimonial_categories/content', $data);
	...
	public function addContent() {
		...
		$this->template->write_view('content','BACKEND/testimonial_categories/edit',$data);
		$this->template->render();
	}
	...
	public function saveContent() {
		if($_POST) {
			...
			if($this->model->saveContent($this->category, $data)){
				...
				header('location:'.PATH_URL.'acp/testimonial/categories/edit/'.$id);
			}
		}
	}
	public function delete() {
		if(isset($_POST['orderButton'])) {
			$this->session->set_flashdata('success', 1);
			header('location:'.PATH_URL.'acp/testimonial/categories');
			exit();
		}
	...
	public function searchContent($category) {
		$data = array(
            'category' => $this->model->getListContent($this->category, array('parent_id' => 0, 'type' => 'page'), "order_by asc", null,null, "name, id"),
			'result' => $this->model->getListContent($this->category, "FIND_IN_SET($category,category_id)>0 AND type = 'testimonial'", "order_by asc", null, null, "images,name, id, status, order_by, category_id, links")
        );
		$this->template->write_view('content','BACKEND/testimonial_categories/content', $data);
		$this->template->render();
	}
	...
	public function checkLinks()
    {
		...
		if($id == '') {
			$order_by = (int) $this->db->get_where($this->category,array('type' => 'testimonial'))->num_rows() + 1;
		} else {
