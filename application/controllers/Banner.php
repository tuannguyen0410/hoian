...
class Banner extends CI_Controller {
	...
	public function admincp() {
		...
		$data = array(
			...
			'result' => $this->model->getListContent($this->article,array('type' => 'banner'), 'category_id asc, order_by asc, updated desc', null, null, "images,name,name_en,id,status,order_by,category_id,links,homepage,featured")
		);
		$this->template->write_view('content','BACKEND/banner/content', $data);
	...
	public function addContent() {
		...
		$this->template->write_view('content','BACKEND/banner/edit',$data);
		$this->template->render();
	}
	...
	public function saveContent() {
		if($_POST) {
			...
			$data = array(
				...
				'shortcontent' => addslashes($this->input->post('shortcontent')),
			...
			if($this->model->saveContent($this->article,$data)){
				...
				echo PATH_URL.'acp/banner/edit/'.$id;
			}
		}
	}
	public function delete() {
		if(isset($_POST['orderButton'])) {
			...
			header('location:'.PATH_URL.'acp/banner');
			exit();
		}
	...
	public function searchContent($category) {
		$data = array(
            'category' => $this->model->getListContent($this->category, array('parent_id' => 0, 'type' => 'menu'), "order_by asc", null,null, "name, id"),
			'result' => $this->model->getListContent($this->article, "FIND_IN_SET($category,category_id)>0 AND type = 'banner'", "order_by asc", null, null, "images,name, id, status, order_by, category_id, links, homepage, featured")
        );
		$this->template->write_view('content','BACKEND/banner/content', $data);
		$this->template->render();
	}
	...
	public function checkLinks()
    {
		...
		if($id == '') {
			$order_by = (int) $this->db->get_where($this->article, array('type' => 'banner'))->num_rows() + 1;
		} else {
