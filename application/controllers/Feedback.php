...
class Feedback extends CI_Controller {
	...
	public function admincp() {
		$data = array(
			'result' => $this->model->getListContent($this->faqs,array('type' => 'feedback'), 'order_by asc')
		);
		$this->template->write_view('content','BACKEND/feedback/content', $data);
	...
	public function addContent() {
		...
		$this->template->write_view('content','BACKEND/feedback/edit',$data);
		$this->template->render();
	}
	...
	public function saveContent() {
		if(!empty($_POST)) {
			...
			$data = array(
				...
				'regency' => trim(addslashes($this->input->post('regency'))),
				'content' => addslashes($this->input->post('content')),
				'name_en' => trim(addslashes($this->input->post('txtName_en'))),
				'regency_en' => trim(addslashes($this->input->post('regency_en'))),
			...
			if($this->model->saveContent($this->faqs,$data)){
				...
				header('location:'.PATH_URL.'acp/feedback/edit/'.$id);
			}
		}
	}
	public function delete() {
		if(isset($_POST['orderButton'])) {
			$this->session->set_flashdata('success', 1);
			header('location:'.PATH_URL.'acp/feedback');
			exit();
		}
