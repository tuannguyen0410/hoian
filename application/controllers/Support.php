...
class Support extends CI_Controller {
	...
	public function admincp() {
		...
		$data = array(
			...
			'result' => $this->model->getListContent($this->article,array('type' => 'support'), 'category_id asc, order_by asc, updated desc', null, null, "images,name,name_en,id,status,order_by,category_id,links,homepage,featured")
		);
		$this->template->write_view('content','BACKEND/support/content', $data);
	...
	public function addContent() {
		...
		$data = array(
			'result' => $result,
			'category' => $this->model->getListContent($this->category, array('parent_id' => 0, 'type' => 'support'), "order_by asc", null,null, "name, id")
		);
		$this->template->write_view('content','BACKEND/support/edit',$data);
		$this->template->render();
	}
	...
	public function delete() {
		if(isset($_POST['orderButton'])) {
			...
			header('location:'.PATH_URL.'acp/support');
			exit();
		}
	...
	public function checkLinks()
    {
		...
		if($id == '') {
			$order_by = (int) $this->db->get_where($this->article, array('type' => 'support'))->num_rows() + 1;
		} else {
