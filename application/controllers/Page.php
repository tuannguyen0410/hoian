...
class Page extends CI_Controller {
	...
	public function admincp() {
		$data = array(
			'result' => $this->model->getListContent($this->category,array('parent_id' => 0, 'type' => 'page'), 'category_id asc, order_by asc')
		);
		$this->template->write_view('content','BACKEND/page/content', $data);
	...
	public function addContent() {
		...
		$this->template->write_view('content','BACKEND/page/edit',$data);
		$this->template->render();
	}
	...
	public function saveContent() {
		if($_POST) {
			...
			$data = array(
				...
				'links_en' => trim('en/'.$this->input->post('links'))
			);
			if($this->model->saveContent($this->category, $data)){
				if($id == '')
					$id = $this->db->insert_id();
				$this->session->set_flashdata('success', 1);
				header('location:'.PATH_URL.'acp/page/edit/'.$id);
			}
		}
	}
	public function delete() {
		if(isset($_POST['orderButton'])) {
			$this->session->set_flashdata('success', 1);
			header('location:'.PATH_URL.'acp/page');
			exit();
		}
	...
	public function checkLinks()
    {
		...
		if($id == '') {
			$order_by = (int) $this->db->get_where($this->category,array('type' => 'page'))->num_rows() + 1;
		} else {
