<h3 class="page-title"><?=@_faqs?> <small><?=@_all?> <?=@_faqs?></small></h3>
<div class="page-bar">
...
<div class="panel-body">
	...
	<form name="frmList" id="frm_content" method="post" action="<?=PATH_URL?>acp/faqs/del" class="form-horizontal" enctype="multipart/form-data">
				...
				<?php if(!empty($result)) { ?>
					<?php $i = 0; foreach($result as $items=>$item) { $i++;?>
						<tr>
							...
							<td>
								<a href="<?=PATH_URL?>acp/faqs/edit/<?=$item->id?>" class="tooltips green btn btn-xs btn_delete_single" data-container="body" data-placement="top" data-original-title="<?=@_edit?>"><i class="fa fa-edit-o"></i> <?=@_edit?> </a>
								<a onclick="return confirm('Are you sure?')" href="<?=PATH_URL?>acp/faqs/del/<?=$item->id?>" class="tooltips red btn btn-xs btn_delete_single" data-container="body" data-placement="top" data-original-title="<?=@_delete?>"><i class="fa fa-trash-o"></i> <?=@_delete?> </a>
							</td>
...
<script>
	...
	function updateStatus(id,type) {
		$.post('<?=PATH_URL?>acp/faqs/updateStatus',{id:id,type:type}, function(data){if(data == 1) location.reload();})
	}
	function changeOrderBy(id,val){
		$.post('<?=PATH_URL?>acp/faqs/changeOrderBy',{id:id, order_by:val},function(data){$("#order_by_"+id).val(data)})
	}
</script>
