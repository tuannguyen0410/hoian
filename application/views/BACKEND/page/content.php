...
<div class="panel-body">
	...
	<form name="frmList" id="frm_content" method="post" action="<?=PATH_URL?>acp/page/del" class="form-horizontal" enctype="multipart/form-data">
		...
		<table class="table table-striped table-bordered table-hover" id="sample_1">
			...
			<tbody>
				<?php if(!empty($result)) { ?>
					...
					<tr>
						<td><?=$i?></td>
						<td style="color: red;">&rarr; <?=stripslashes($item->name)?></td>
						<td><?=$item->links?></td>
						...
						<td>
							<a href="<?=PATH_URL?>acp/page/edit/<?=$item->id?>" class="tooltips yellow btn btn-xs " data-container="body" data-placement="top" data-original-title="<?=@_edit?>"><i class="fa fa-pencil"></i>  <?=@_edit?> </a>
							<a href="<?=PATH_URL?>acp/page/del/<?=$item->id?>" class="tooltips red btn btn-xs btn_delete_single" data-container="body" data-placement="top" data-original-title="<?=@_delete?>"><i class="fa fa-trash-o"></i> <?=@_delete?> </a>
						</td>
...
<script>
	...
	function updateStatus(id,type) {
		$.post('<?=PATH_URL?>acp/page/updateStatus',{id:id,type:type}, function(data){if(data == 1) location.reload();})
	}
	function changeOrderBy(id,val){
		$.post('<?=PATH_URL?>acp/page/changeOrderBy',{id:id, order_by:val},function(data){$("#order_by_"+id).val(data)})
	}
</script>
