...
<h3 class="page-title">
	Network <small><?=@_create_edit?> Network</small>
</h3>
<?php if($this->uri->segment(3) == 'edit') { ?>
    <div class="form-group">
        <button id="btn_submit" type="button" class="btn btn-success" onclick="return location.href='<?=PATH_URL?>acp/network/add'"><?=@_add_new?> <i class="fa fa-plus"></i></button>
</div>
<?php } ?>
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?=PATH_URL?>acp/network">Network</a>
			<i class="fa fa-angle-right"></i>
		</li>
		<li>
...
<div class="panel-body">
	<form id="frm_submit" method="post" action="<?=PATH_URL?>acp/network/save" class="form-horizontal" enctype="multipart/form-data">
...
<!--						<li>
					...
					<div class="tab-content">
						<div class="tab-pane active" id="tab_1_1">
							...
							<div class="form-group">
								<label class="col-sm-2 control-label"><?=@_address?></label>
								<div class="col-sm-10">
									<input type="text" class="form-control" required autocomplete="off" id="txtAddress" name="txtAddress" value="<?php if(!empty($result)) echo stripslashes($result->shortcontent);?>" placeholder="<?=@_address?>">
								</div>
							</div>
                            
							<div class="form-group">
								<label class="col-sm-2 control-label"><?=@_coordinates?></label>
								<div class="col-sm-10">
									<input type="text" class="form-control" required autocomplete="off" id="txtCoordinates" name="txtCoordinates" value="<?php if(!empty($result)) echo stripslashes($result->title);?>" placeholder="10.12346,106.43522">
								</div>
							</div>

							<div class="form-group">
								<label class="col-sm-2 control-label"><?=@_phone?></label>
								<div class="col-sm-10">
									<input type="text" class="form-control" required autocomplete="off" id="txtPhone" name="txtPhone" value="<?php if(!empty($result)) echo stripslashes($result->description);?>" placeholder="<?=@_phone?>">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<input type="url" class="form-control" id="links" name="links" value="<?php if(!empty($result)) echo $result->links;?>" placeholder="Url">
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label"><?=@_discription;?></label>
								<div class="col-sm-10">
									<textarea class="form-control" id="content" name="content"><?php if(!empty($result)) echo $result->content;?></textarea>
								</div>
							</div>
                            <div class="form-group">
						...
						<div class="tab-pane" id="tab_1_2">
							...
							<div class="form-group">
								<label class="col-sm-2 control-label"><?=@_address?></label>
								<div class="col-sm-10">
									<input type="text" class="form-control" required autocomplete="off" id="txtAddress_en" name="txtAddress_en" value="<?php if(!empty($result)) echo stripslashes($result->shortcontent_en);?>" placeholder="<?=@_address?>">
								</div>
							</div>
							...
							<div class="form-group">
								<label class="col-sm-2 control-label">Video Url</label>
								<div class="col-sm-10">
									<input type="url" class="form-control" id="links_en" name="links_en" value="<?php if(!empty($result)) echo $result->links_en;?>" placeholder="Url">
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-2 control-label"><?=@_discription;?></label>
								<div class="col-sm-10">
									<textarea class="form-control" id="content_en" name="content_en"><?php if(!empty($result)) echo $result->content_en;?></textarea>
								</div>
							</div>
						</div>
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-10">
						...
						<button type="button" onclick="return location.href='<?=PATH_URL?>acp/network'" class="btn btn-info"><i class="fa fa-ban"></i> <?=@_cancel?></button>
					</div>
				</div>
			</div>
							...
							<ul class="list-unstyled">
								<?php if(!empty($result)) $result_id = $result->id; else $result_id = 0; foreach($category as $items=>$item) { ?>
									<li>
										...
										<?php $child_category = $this->model->getListContent('category', "FIND_IN_SET(".$item->id.",category_id)>0 AND type='network' AND id <> $result_id AND parent_id = 0",'order_by asc', null, null, "name, id, parent_id");?>
											...
											<ul class="list-unstyled">
												<?php foreach ($child_category as $items1 => $item1) { ?>
													<li>
														...
														<?php $child1 = $this->model->getListContent('category', array('parent_id' => $item1->id, 'type' => 'network', 'id <> ' => $result_id),'order_by asc', null, null, "name, id");?>
														<?php if(!empty($child1)) { ?>
															<ul class="list-unstyled">
...
<script>
	...
	function delGalleryImage(id) {
		$.post("<?=PATH_URL?>acp/network/delGalleryImage",{id:id},function(data){$("#images_content").html(data)})
	}
	function deleteImageContent(id) {
		$.post("<?=PATH_URL?>acp/network/deleteImageContent",{id:id},function(data){$("#imgContent").html(data); location.reload();});
	}
    ...
    function checkLinks(name){
        ...
        $.post("<?=PATH_URL?>acp/network/checkLinks",{links:links,id:$("#id").val()},function(data){var json = JSON.parse(data); $("#links").val(json.links)});
    }
