...
<h3 class="page-title">
	Banner <small><?=@_create_edit?> Banner</small>
</h3>
<?php if($this->uri->segment(3) == 'edit') { ?>
    <div class="form-group">
        <button id="btn_submit" type="button" class="btn btn-success" onclick="return location.href='<?=PATH_URL?>acp/banner/add'"><?=@_add_new?> <i class="fa fa-plus"></i></button>
</div>
<?php } ?>
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?=PATH_URL?>acp/banner">Banner</a>
			<i class="fa fa-angle-right"></i>
		</li>
		<li>
...
<div class="panel-body">
	<form id="frm_submit" method="post" action="<?=PATH_URL?>acp/banner/save" class="form-horizontal" enctype="multipart/form-data">
			...
			<div class="col-md-9 col-sm-9">
					...
					<div class="tab-content">
						<div class="tab-pane active" id="tab_1_1">
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" autocomplete="off" id="shortcontent" name="shortcontent" value="<?php if(!empty($result)) echo stripslashes($result->shortcontent);?>" placeholder="<?=@_workplace?>">
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<input type="text" class="form-control" id="links" name="links" value="<?php if(!empty($result)) echo $result->links;?>" placeholder="Url">
								</div>
							</div>
							<div class="form-group">
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-10">
						...
						<button type="button" onclick="return location.href='<?=PATH_URL?>acp/banner'" class="btn btn-info"><i class="fa fa-ban"></i> <?=@_cancel?></button>
					</div>
				</div>
			</div>
							...
							<ul class="list-unstyled">
								<?php if(!empty($result)) $result_id = $result->id; else $result_id = 0; foreach($category as $items=>$item) { ?>
									<li>
										...
										<?php $child_category = $this->model->getListContent('category', "FIND_IN_SET(".$item->id.",category_id)>0 AND type='banner' AND id <> $result_id AND parent_id = 0",'order_by asc', null, null, "name, id, parent_id");?>
...
<script>
	...
	function delGalleryImage(id) {
		$.post("<?=PATH_URL?>acp/banner/delGalleryImage",{id:id},function(data){$("#images_content").html(data)})
	}
	function deleteImageContent(id) {
		$.post("<?=PATH_URL?>acp/banner/deleteImageContent",{id:id},function(data){$("#imgContent").html(data); location.reload();});
	}
    ...
    function checkLinks(name){
        ...
        $.post("<?=PATH_URL?>acp/banner/checkLinks",{links:links,id:$("#id").val()},function(data){var json = JSON.parse(data); $("#links").val(json.links)});
    }
