<h3 class="page-title">Banner <small><?=@_all?> Banner</small></h3>
<div class="page-bar">
...
<div class="panel-body">
	<form method="post" class="form-inline" style="margin-bottom: 20px;">
            ...
            <select class="form-control input-large" name="category">
                ...
                <?php if(!empty($category)) { ?>
                    <?php foreach($category as $items=>$item) { ?>
                        ...
                        <?php $child_category = $this->model->getListContent('category', "FIND_IN_SET(".$item->id.",category_id)>0 AND type='banner'", "order_by asc", null,null, "name, id")?>
        ...
        <div class="form-group">
            <button type="button" onclick="return location.href='<?=PATH_URL?>acp/banner/filter/'+$('select').val()" id="btn_search" class="btn btn-info"><i class="icon-search"></i> <?=@_filter?></button>
        </div>
        <div class="form-group navbar-right" style="margin-right: 0;">
			<button id="btn_submit" type="button" class="btn btn-success" onclick="return location.href='<?=PATH_URL?>acp/banner/add'"><i class="icon-plus"></i> <?=@_add_new?></button>
		</div>
	</form>
	<form name="frmList" id="frm_content" method="post" action="<?=PATH_URL?>acp/banner/del" class="form-horizontal" enctype="multipart/form-data">
			...
			<tbody>
				<?php if(!empty($result)) { ?>
					<?php $i = 0; foreach($result as $items=>$item) { $i++; $category_name = $this->model->getContent('category', "id IN ($item->category_id)","name");?>
						<tr>
							<td><a href="<?=PATH_URL?>acp/banner/edit/<?=$item->id?>"><img src="<?=PATH_URL?>media/<?php if($item->images <> null) echo $item->images; else echo "no_images.gif";?>" class="img-thumbnail" style="max-width: 100px;"></a></td>
							...
							<td>
								<a href="<?=PATH_URL?>acp/banner/edit/<?=$item->id?>" class="tooltips yellow btn btn-xs " data-container="body" data-placement="top" data-original-title="<?=@_edit?>"><i class="fa fa-pencil"></i>  <?=@_edit?> </a>
								<a onclick="return confirm('Are you sure?')" href="<?=PATH_URL?>acp/banner/del/<?=$item->id?>" class="tooltips red btn btn-xs btn_delete_single" data-container="body" data-placement="top" data-original-title="<?=@_delete?>"><i class="fa fa-trash-o"></i> <?=@_delete?> </a>
							</td>
...
<script>
	...
	function updateStatus(id,type) {
		$.post('<?=PATH_URL?>acp/banner/updateStatus',{id:id,type:type}, function(data){if(data == 1) location.reload();})
	}
	function changeOrderBy(id,val){
		$.post('<?=PATH_URL?>acp/banner/changeOrderBy',{id:id, order_by:val},function(data){$("#order_by_"+id).val(data)})
	}
</script>
