<h3 class="page-title">Tour <small><?=@_all?> Tour</small></h3>
<div class="page-bar">
...
<div class="panel-body">
	<form method="post" class="form-inline" style="margin-bottom: 20px;">
            ...
            <select class="form-control input-large" name="category">
                ...
                <?php if(!empty($category)) { ?>
                    <?php foreach($category as $items=>$item) { ?>
                        ...
                        <?php $child_category = $this->model->getListContent('category', "FIND_IN_SET(".$item->id.",category_id)>0 AND type='tour'", "order_by asc", null,null, "name, id")?>
        ...
        <div class="form-group">
            <button type="button" onclick="return location.href='<?=PATH_URL?>acp/tour/filter/'+$('select').val()" id="btn_search" class="btn btn-info"><i class="icon-search"></i> <?=@_filter?></button>
        </div>
        <div class="form-group navbar-right" style="margin-right: 0;">
			<button id="btn_submit" type="button" class="btn btn-success" onclick="return location.href='<?=PATH_URL?>acp/tour/add'"><i class="icon-plus"></i> <?=@_add_new?></button>
		</div>
	</form>
		...
		<table class="table table-striped table-bordered table-hover" id="sample_1">
			...
			<tbody>
				<?php if(!empty($result)) { ?>
					...
					$category_name = $this->model->getListContent('category', "id IN ($item->category_id)","name"); 
					?>
						<tr>
							<td><a href="<?=PATH_URL?>acp/tour/edit/<?=$item->id?>"><img src="<?=PATH_URL?>media/<?php if($item->images <> null) echo $item->images; else echo "no_images.gif";?>" class="img-thumbnail" style="max-width: 100px;"></a></td>
							<td><?=stripslashes($item->name)?><br /><b>Price: $<?=stripslashes($item->title)?></b></td>
							<td>  
							<?php if(!empty($category_name)){ foreach($category_name as $cat){ ?>
                            	<?php echo $cat->name.', '; ?>  
                            <?php }} ?>
                            </td>
							<td align="center">
							...
							<td>
								<a href="<?=PATH_URL?>acp/tour/edit/<?=$item->id?>" class="tooltips yellow btn btn-xs " data-container="body" data-placement="top" data-original-title="<?=@_edit?>"><i class="fa fa-pencil"></i>  <?=@_edit?> </a>
								<a onclick="return confirm('Are you sure?')" href="<?=PATH_URL?>acp/tour/del/<?=$item->id?>" class="tooltips red btn btn-xs btn_delete_single" data-container="body" data-placement="top" data-original-title="<?=@_delete?>"><i class="fa fa-trash-o"></i> <?=@_delete?> </a>
							</td>
...
<script>
	...
	function updateStatus(id,type) {
		$.post('<?=PATH_URL?>acp/tour/updateStatus',{id:id,type:type}, function(data){if(data == 1) location.reload();})
	}
	function changeOrderBy(id,val){
		$.post('<?=PATH_URL?>acp/tour/changeOrderBy',{id:id, order_by:val},function(data){$("#order_by_"+id).val(data)})
	}
</script>
