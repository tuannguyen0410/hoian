<h3 class="page-title"><?=@_article?> <small><?=@_all?> <?=@_article?></small></h3>
<div class="page-bar">
...
<div class="panel-body">
	<form method="post" class="form-inline" style="margin-bottom: 20px;">
        ...
        <div class="form-group">
            <button type="button" onclick="return location.href='<?=PATH_URL?>acp/article/filter/'+$('select').val()" id="btn_search" class="btn btn-info"><i class="icon-search"></i> <?=@_filter?></button>
        </div>
	...
	<form name="frmList" id="frm_content" method="post" action="<?=PATH_URL?>acp/article/del" class="form-horizontal" enctype="multipart/form-data">
		<table class="table table-striped table-bordered table-hover" id="sample_1">
			<thead class="flip-content">
				<tr>
					<th><?=@_image?></th>
					<th style="min-width: 200px;"><?=@_name?></th>
					<th><?=@_category?></th>
					<th style="min-width: 70px;" align="center">
						<i class="fa fa-home"></i>
			...
			<tbody>
				<?php if(!empty($result)) { 
					foreach($result as $items=>$item) {
						$category_name = $this->model->getContent('category', "FIND_IN_SET(id,'".$item->category_id."')>0 AND type = 'article'","name");
				?>
					<tr>
						<td><a href="<?=PATH_URL?>acp/article/edit/<?=$item->id?>"><img src="<?=PATH_URL?>media/<?php if($item->images <> null) echo $item->images; else echo "no_images.gif";?>" class="img-thumbnail" style="max-width: 100px;"></a></td>
						...
						<td align="center">
							...
							<?php if($item->featured == 1) { ?><i style="cursor: pointer; color: green;" onclick="updateStatus('<?=$item->id?>','featured')" class="fa fa-star"></i><?php } else { ?><i style="cursor: pointer; color: red;" onclick="updateStatus('<?=$item->id?>','featured')" class="fa fa-star-o"></i><?php } ?>
						</td>
						...
						<td>
							<a href="<?=PATH_URL?>acp/article/edit/<?=$item->id?>" class="tooltips yellow btn btn-xs " data-container="body" data-placement="top" data-original-title="<?=@_edit?>"><i class="fa fa-pencil"></i>  <?=@_edit?> </a>
							<a onclick="return confirm('Are you sure?')" href="<?=PATH_URL?>acp/article/del/<?=$item->id?>" class="tooltips red btn btn-xs btn_delete_single" data-container="body" data-placement="top" data-original-title="<?=@_delete?>"><i class="fa fa-trash-o"></i> <?=@_delete?> </a>
						</td>
...
<script>
	...
	function updateStatus(id,type) {
		$.post('<?=PATH_URL?>acp/article/updateStatus',{id:id,type:type}, function(data){if(data == 1) location.reload();})
	}
	function changeOrderBy(id,val){
		$.post('<?=PATH_URL?>acp/article/changeOrderBy',{id:id, order_by:val},function(data){$("#order_by_"+id).val(data)})
	}
</script>
