...
<h3 class="page-title">
	<?=@_article?> <small><?=@_create_edit?> <?=@_article?></small>
</h3>
<?php if($this->uri->segment(3) == 'edit') { ?>
    <div class="form-group">
        <button id="btn_submit" type="button" class="btn btn-success" onclick="return location.href='<?=PATH_URL?>acp/article/add'"><?=@_add_new?> <i class="fa fa-plus"></i></button>
</div>
<?php } ?>
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?=PATH_URL?>acp/article"><?=@_article?></a>
			<i class="fa fa-angle-right"></i>
		</li>
		<li>
...
<div class="panel-body">
	<form method="post" action="<?=PATH_URL?>acp/article/save" class="form-horizontal" enctype="multipart/form-data">
			...
			<div class="col-md-9 col-sm-9">
				<div class="tabbable-custom ">
					<ul class="nav nav-tabs ">
						...
						<!--<li>
							<a href="#tab_1_6" data-toggle="tab"><?=@_content_en?></a>
						</li>-->
						<li>
					...
					<div class="tab-content">
						<div class="tab-pane active" id="tab_1_1">
							<div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" required autocomplete="off" id="txtName" onkeyup="addLinks(this.value,'')" name="txtName" value="<?php if(!empty($result)) echo stripslashes($result->name);?>" placeholder="<?=@_name?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-2 control-label"><?=@_short_content?></label>
                                <div class="col-sm-10">
                                    <textarea class="form-control" rows="7" id="shortcontent" name="shortcontent"><?php if(!empty($result)) echo stripslashes($result->shortcontent);?></textarea>
                                </div>
                            </div>
                            <div class="form-group">
							...
							<div class="form-group">
								<label class="col-sm-2 control-label"><?=@_tags?></label>
								<div class="col-sm-10">
						...
						<div class="tab-pane" id="tab_1_6">
                            ...
                            <div class="form-group">
                                <label class="col-sm-2 control-label"><?=@_short_content?></label>
                                <div class="col-sm-10">
                                    <textarea class="form-control" rows="7" id="shortcontent_en" name="shortcontent_en"><?php if(!empty($result)) echo stripslashes($result->shortcontent_en)?></textarea>
							...
							<div class="form-group">
								<label class="col-sm-2 control-label"><?=@_slug?></label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="links_en" name="links_en" value="<?php if(!empty($result)) echo str_replace('en/','',$result->links_en);?>" placeholder="<?=@_slug?>">
								</div>
							</div>
						</div>
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-10">
						...
						<button type="button" onclick="return location.href='<?=PATH_URL?>acp/article'" class="btn btn-info"><i class="fa fa-ban"></i> <?=@_cancel?></button>
					</div>
				</div>
			</div>
						...
						<div class="scroller" style="height:250px" data-always-visible="1" data-rail-visible="1" data-rail-color="blue" data-handle-color="green">
							<ul class="list-unstyled">
								<?php if(!empty($result)) $result_id = $result->id; else $result_id = 0; foreach($category as $items=>$item) { ?>
									<li>
										<label><input type="radio" name="category_id" value="<?=$item->id?>" <?php if(!empty($result)) { if(in_array($item->id, explode(',',$result->category_id))) echo "checked";}?>><?=stripslashes($item->name)?></label>
										<?php $child_category = $this->model->getListContent('category', "FIND_IN_SET(".$item->id.",category_id)>0 AND type='article' AND id <> $result_id AND parent_id = 0",'order_by asc', null, null, "name, id");?>
										<?php if(!empty($child_category)) { ?>
											<ul class="list-unstyled">
												<?php foreach ($child_category as $items1 => $item1) { ?>
													<li>
														<label><input type="radio" name="category_id" value="<?=$item1->id?>" <?php if(!empty($result)) { if(in_array($item1->id, explode(',',$result->category_id))) echo "checked";}?>><?=stripslashes($item1->name)?></label>
														<?php $child1 = $this->model->getListContent('category', array('parent_id' => $item1->id, 'type' => 'article', 'id <> ' => $result_id),'order_by asc', null, null, "name, id");?>
														<?php if(!empty($child1)) { ?>
															<ul class="list-unstyled">
																<?php foreach ($child1 as $items2 => $item2) { ?>
																	<li>
																		<label><input type="radio" name="category_id" value="<?=$item2->id?>" <?php if(!empty($result)) { if(in_array($item2->id, explode(',',$result->category_id))) echo "checked";}?>><?=stripslashes($item2->name)?></label>
																	</li>
																<?php }?>
															</ul>
														<?php } ?>
...
<script>
	$(function(){
		...
		$("#thebox").bind('change',function(){
			...
			var ext = name.split(".");
            var type = ['image/jpeg','image/gif','image/png','image/bmp']
			...
			var chk_ext = ['png','gif', 'jpg', 'jpeg']
			if($.inArray(ext.toLowerCase(),chk_ext) < 0) {
				this.value = "";
    ...
    function checkLinks(name,lang){
      ...
      $.post("<?=PATH_URL?>acp/article/checkLinks",{links:links,lang:lang,id:$("#id").val()},function(data){var json = JSON.parse(data); $("#links"+lang).val(json.links)});
    }
