...
<div class="panel-body">
	<form method="post" class="form-inline" style="margin-bottom: 20px;">
        ...
        <div class="form-group">
            <button type="button" onclick="return location.href='<?=PATH_URL?>acp/banner/categories/filter/'+$('select').val()" id="btn_search" class="btn btn-info"><i class="icon-search"></i> <?=@_filter?></button>
        </div>
        <div class="form-group navbar-right" style="margin-right: 0;">
			<button id="btn_submit" type="button" class="btn btn-success" onclick="return location.href='<?=PATH_URL?>acp/banner/categories/add'"><i class="icon-plus"></i> <?=@_add_new?></button>
		</div>
	</form>
	<form name="frmList" id="frm_content" method="post" action="<?=PATH_URL?>acp/banner/categories/del" class="form-horizontal" enctype="multipart/form-data">
				...
				<?php 
					if(!empty($result)) { 
						foreach($result as $items=>$item) {
							...
							$child = $this->model->getListContent('category', array('parent_id' => $item->id, 'type' => 'banner'), 'order_by asc', null, null, "name, images, id, status, order_by");
				?>
					<tr>
						...
						<td>
							<a href="<?=PATH_URL?>acp/banner/categories/edit/<?=$item->id?>" class="tooltips yellow btn btn-xs " data-container="body" data-placement="top" data-original-title="<?=@_edit?>"><i class="fa fa-pencil"></i>  <?=@_edit?> </a>
							<a onclick="return confirm('Are you sure?')" href="<?=PATH_URL?>acp/banner/categories/del/<?=$item->id?>" class="tooltips red btn btn-xs btn_delete_single" data-container="body" data-placement="top" data-original-title="<?=@_delete?>"><i class="fa fa-trash-o"></i> <?=@_delete?> </a>
						</td>
					...
					?>
						<tr>
							...
							<td>
								<a href="<?=PATH_URL?>acp/banner/categories/edit/<?=$item1->id?>" class="tooltips yellow btn btn-xs " data-container="body" data-placement="top" data-original-title="<?=@_edit?>"><i class="fa fa-pencil"></i>  <?=@_edit?> </a>
								<a onclick="return confirm('Are you sure?')" href="<?=PATH_URL?>acp/banner/categories/del/<?=$item1->id?>" class="tooltips red btn btn-xs btn_delete_single" data-container="body" data-placement="top" data-original-title="<?=@_delete?>"><i class="fa fa-trash-o"></i> <?=@_delete?> </a>
							</td>
...
<script>
	...
	function updateStatus(id,type) {
		$.post('<?=PATH_URL?>acp/banner/categories/updateStatus',{id:id,type:type}, function(data){if(data == 1) location.reload();})
	}
	function changeOrderBy(id,val){
		$.post('<?=PATH_URL?>acp/banner/categories/changeOrderBy',{id:id, order_by:val},function(data){$("#order_by_"+id).val(data)})
	}
</script>
