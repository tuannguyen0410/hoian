...
<?php if($this->uri->segment(4) == 'edit') { ?>
    <div class="form-group">
        <button id="btn_submit" type="button" class="btn btn-success" onclick="return location.href='<?=PATH_URL?>acp/banner/categories/add'"><?=@_add_new?> <i class="fa fa-plus"></i></button>
</div>
<?php } ?>
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?=PATH_URL?>acp/banner/categories"><?=@_category?></a>
			<i class="fa fa-angle-right"></i>
		</li>
		<li>
...
<div class="panel-body">
	<form id="frm_submit" method="post" action="<?=PATH_URL?>acp/banner/categories/save" class="form-horizontal" enctype="multipart/form-data">
			...
			<div class="col-md-9 col-sm-9">
				...
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-10">
						...
						<button type="button" onclick="return location.href='<?=PATH_URL?>acp/banner/categories'" class="btn btn-info"><i class="fa fa-ban"></i> <?=@_cancel?></button>
					</div>
				</div>
			</div>
							...
							<ul class="list-unstyled">
								<?php if(!empty($result)) $result_id = $result->id; else $result_id = 0; foreach($category as $items=>$item) { ?>
									<li>
										...
										<?php $child_category = $this->model->getListContent('category', "FIND_IN_SET(".$item->id.",category_id)>0 AND type='banner' AND id <> $result_id AND parent_id = 0",'order_by asc', null, null, "name, id");?>
