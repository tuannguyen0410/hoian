<h3 class="page-title"><?=@_booking?> <small><?=@_all?> <?=@_booking?></small></h3>
<div class="page-bar">
...
<div class="panel-body">
	<form name="frmList" id="frm_content" method="post" action="<?=PATH_URL?>acp/booking/del" class="form-horizontal" enctype="multipart/form-data">
				...
				<?php if(!empty($result)) { ?>
					<?php $i = 0; foreach($result as $items=>$item) { $i++;?>
						<tr>
							...
							<td><a href="<?=PATH_URL?>acp/booking/edit/<?=$item->id?>" title="<?=@_edit?>"><i class="fa <?php if($item->status == 0) echo "fa-eye-slash"; else echo "fa-eye";?>"></i></a></td>
							<td>
								<a onclick="return confirm('Are you sure?')" href="<?=PATH_URL?>acp/booking/del/<?=$item->id?>" class="tooltips red btn btn-xs btn_delete_single" data-container="body" data-placement="top" data-original-title="<?=@_delete?>"><i class="fa fa-trash-o"></i> <?=@_delete?> </a>
							</td>
